...
- Preserve source weight perturbation of +/-20% per dimension, weight-vector sum 1.0 with +/-0.001 tolerance, at least 4 scenarios, and stability labels `stable`/`sensitive`/`unstable` with $\tau$ bands >=0.8, 0.5-<0.8, and <0.5.
- Rank comparison reports at least one Kendall $\tau$ or Spearman $\rho$ and all alternatives differing by >=2 positions.
...
- `weight-perturbation` line 35: stable (all scenarios $\tau$ >= 0.8) / sensitive (any scenario 0.5 <= $\tau$ < 0.8) / unstable (any scenario $\tau$ < 0.5)
...
| hypothesis-formation/weight-perturbation | 21 | numeric | Apply +/-20% perturbations per dimension; stable means all $\tau$ >=0.8, sensitive means any 0.5<=$\tau$<0.8, unstable means any $\tau$<0.5. |
