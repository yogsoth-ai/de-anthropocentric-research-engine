...
- **Claude group** (for the three CC layers optimizer / sim / exec):
  ...
  - `ANTHROPIC_BASE_URL = https://<PROXY_HOST>` (no suffix; the CLI appends `/v1/messages` itself)
  - `ANTHROPIC_MODEL = claude-opus-4-7`
- **Codex group** (for the codex used by the two loss computations):
  - `OPENAI_API_KEY / PROXY_KEY = *****` (the Codex group's dedicated key)
  - `OPENAI_BASE_URL = https://<PROXY_HOST>/v1`
