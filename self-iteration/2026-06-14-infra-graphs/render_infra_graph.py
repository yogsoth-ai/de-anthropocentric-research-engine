...
def strip_chrome(html: str) -> str:
    """Remove pyvis's bootstrap card frame and fill the viewport; replace vis's
    native (plain-text) tooltip with a self-managed #dare-tip layer that renders
    the HTML title on hoverNode / hoverEdge events — robust across vis versions."""
    html = html.replace("</head>", _CHROME_CSS + "</head>", 1)
    inject = """
  // self-managed HTML tooltip: stash titles, clear vis native title, render on hover
