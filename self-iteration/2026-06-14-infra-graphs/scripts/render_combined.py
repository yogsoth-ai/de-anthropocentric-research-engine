...
def load_collisions():
    """29 cross-package collision groups; each {pkg, old} renames the same way
    as a wrapper: <pkg>/<old> -> <pkg>/<pkg>-<old>. File generated by
    gen_collision_links.py."""
    p = HERE / "collision-links.json"
    return json.loads(p.read_text(encoding="utf-8"))["groups"] if p.exists() else []


def build_rename(links):
    """wrapper `<pkg>/<wrapper>` AND collision `<pkg>/<old>` -> renamed canonical
    id `<pkg>/<pkg>-<name>`. Both use the identical rename shape, so they share
    one map; canon()/node-loop/edge-loop need no change."""
    rename = {f"{c['pkg']}/{c['wrapper']}": f"{c['pkg']}/{c['pkg']}-{c['wrapper']}"
              for c in links["collapse"]}
    for c in load_collisions():
        rename[f"{c['pkg']}/{c['old']}"] = f"{c['pkg']}/{c['pkg']}-{c['old']}"
    return rename
...
def build(files, links, a_data_dir):
    ...
    for f in ordered:
        ...
        for n in s["nodes"]:
            ...
            if n.get("desc"):
                title += f"<hr>{n['desc']}"
            if key in rename:  # a renamed node (wrapper OR collision)
                if key in target:   # import wrapper
                    tgt = target[key]
                ...
                else:               # cross-package collision rename
                    title += (f'<hr><b>RENAME 计划:</b> 撞名 skill <code>{key}</code> → '
                              f'重命名为 <code>{cid.split("/")[-1]}</code>(裸名 '
                              f'<code>{n["id"]}</code> 跨 package 撞车)。后续 refactory '
                              f'轮据此重命名/新建磁盘 skill 目录。')
    ...
    for key, nid in rename.items():
        if key not in target:   # collision renames have no import target
            continue
        tgt = target[key]
