...
Coordinate structured consensus with explicit dependencies, evidence, uncertainty, and decision boundaries.
...
1. Freeze the declared inputs and establish the decision target. (`map-disagreement`)
2. Transform the current artifact while preserving its provenance and uncertainty. (`run-convergence-round`)
3. Transform the current artifact while preserving its provenance and uncertainty. (`calibrate-probability-forecast`)
4. Integrate the preceding artifacts and state the stopping rationale. (`set-threshold`)
...
- resolved: structured-consensus <- v3 refactory_source.json nodes[].name (normalized lookup)
- concept: structured/consensus <- architecture semantic consolidation
