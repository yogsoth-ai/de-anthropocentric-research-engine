...
Coordinate decompose research question with explicit dependencies, evidence, uncertainty, and decision boundaries.
...
1. Freeze the declared inputs and establish the decision target. (`generate-subquestions`)
2. Transform the current artifact while preserving its provenance and uncertainty. (`map-dependencies`)
3. Integrate the preceding artifacts and state the stopping rationale. (`sequence-work`)
...
- resolved: decompose-research-question <- v3 refactory_source.json nodes[].name (normalized lookup)
- concept: decompose/research/question <- architecture semantic consolidation
