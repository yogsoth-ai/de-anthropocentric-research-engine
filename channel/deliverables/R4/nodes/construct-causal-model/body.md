...
Coordinate construct causal model with explicit dependencies, evidence, uncertainty, and decision boundaries.
...
1. Freeze the declared inputs and establish the decision target. (`identify-variables`)
2. Transform the current artifact while preserving its provenance and uncertainty. (`extract-causal-structure`)
3. Transform the current artifact while preserving its provenance and uncertainty. (`represent-mechanism-edge`)
4. Transform the current artifact while preserving its provenance and uncertainty. (`attach-evidence-to-relation`)
5. Transform the current artifact while preserving its provenance and uncertainty. (`detect-feedback-loop`)
6. Integrate the preceding artifacts and state the stopping rationale. (`analyze-intervention`)
...
- resolved: construct-causal-model <- v3 refactory_source.json nodes[].name (normalized lookup)
- concept: construct/causal/model <- architecture semantic consolidation
