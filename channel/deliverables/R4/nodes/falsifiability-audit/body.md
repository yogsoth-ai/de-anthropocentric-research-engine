...
Coordinate falsifiability audit with explicit dependencies, evidence, uncertainty, and decision boundaries.
...
1. Freeze the declared inputs and establish the decision target. (`evaluate-falsifiability`)
2. Transform the current artifact while preserving its provenance and uncertainty. (`operationalize-construct`)
3. Integrate the preceding artifacts and state the stopping rationale. (`specify-boundaries`)
...
- resolved: falsifiability-audit <- v3 refactory_source.json nodes[].name (normalized lookup)
- concept: falsifiability/audit <- architecture semantic consolidation
