# hypothesis-formation — skill table

72 skills. Sorted by layer (campaign→strategy→tactic→sop), then name.
...
| strategy | comparative-formulation | "Strategy: 构建对比性研究问题 — A vs B 的系统化比较" |
| strategy | competing-hypothesis-construction | "Strategy: 为同一现象构建多个竞争假设" |
| strategy | decomposition-formulation | "Strategy: 将复杂研究问题分解为可独立回答的子问题层级" |
| strategy | deductive-hypothesis-generation | "Strategy: 从现有理论演绎推导假设" |
| strategy | evidence-based-prioritization | "Strategy: 基于证据强度的 AHRQ PiCMe 评估——用文献证据质量驱动 gap 优先级" |
| strategy | feasibility-constrained-formulation | "Strategy: 在资源约束下重塑研究问题 — pragmatic 调整保持核心价值" |
| strategy | framework-guided-formulation | "Strategy: 选择 RQ 框架（PICO/SPIDER/SPICE/ECLIPSE）并系统应用" |
...
| tactic | anomaly-driven-abduction | "Tactic: 归纳/溯因路径——描述异常现象，生成候选解释，按可信度排序" |
| tactic | competing-hypothesis-matrix | "Tactic: 多假设管理——生成竞争假设，设计区分性预测，构建结构化比较矩阵" |
| tactic | falsifiability-audit | "Tactic: 假设质量保证——检验可证伪性，修复不合格假设，完成操作化与边界条件规范" |
| tactic | framework-selection-and-application | "Tactic: 选择最适合的 RQ 框架并系统应用" |
...
| sop | ahrq-picme-assessment | "SOP: 使用 AHRQ PiCMe 框架对研究 gap 进行 6 维度系统评估" |
| sop | anomaly-characterization | "SOP: 描述和分类无法被现有理论解释的异常现象" |
| sop | answering-sequence-design | "SOP: 设计子问题的最优回答顺序" |
| sop | boundary-condition-specification | "SOP: 指定假设成立的边界条件" |
| sop | competing-hypothesis-generation | "SOP: 为同一现象生成机制上不同的竞争假设" |
| sop | consistency-check | "SOP: 检验 pairwise 判断矩阵的传递一致性，识别不一致项并建议修正" |
| sop | dependency-mapping | "SOP: 映射子问题间的依赖关系" |
| sop | discriminating-prediction-design | "SOP: 设计能区分竞争假设的关键预测和观察方案" |
| sop | eclipse-application | "SOP: 应用 ECLIPSE 框架结构化混合方法研究问题" |
| sop | explanation-generation | "SOP: 为异常现象生成候选解释列表" |
| sop | falsifiability-check | "SOP: 检验假设是否满足可证伪性标准" |
| sop | feasibility-scoring | "SOP: 评估研究 gap 的可攻击性，识别瓶颈并输出可行性评分" |
| sop | finer-criteria-check | "SOP: FINER 5 项标准逐项检验研究问题质量" |
| sop | framework-matching | "SOP: 根据研究类型匹配最适合的 RQ 框架" |
| sop | gap-normalization | "SOP: 统一不同来源的 gap 格式为标准 GapRecord" |
| sop | gap-pairwise-judgment | "SOP: 对两个 gap 进行逐标准相对优先级判断，输出偏好结果" |
| sop | hypothesis-comparison-matrix | "SOP: 构建竞争假设的多维度对比矩阵" |
| sop | hypothesis-synthesis | "SOP: 综合所有中间产物，产出最终结构化假设集" |
